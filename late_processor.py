...
if __name__ == "__main__":
    ...
    print("The number of unique words in tweets data set is %i." %len(words_stat.keys()))
    lowTF_words = set()
    with open(os.path.join(data_dir, 'words_statistics_tweet.txt'), 'w', encoding='utf-8') as f:
        f.write('TF\tDF\tWORD\n')
    ...
    print("The number of low frequency words of tweets is %d." %len(lowTF_words))
    # print(stops)         
    ...
    print("Tweets preprocessing is completed")
    ...
    with open(os.path.join(data_dir, x_filename), encoding='utf-8') as f:
        for i, line in enumerate(f):
            ...
            words = pre_process(description, porter)
            for word in words:
                if word not in stops:
                    postprocess_tweet.append(word)
                    if word in words_stat.keys():
                        ...
                        words_stat[word] = [1,1,i]
            descriptions.append(' '.join(postprocess_tweet))
    ##saving the statistics of tf and df for each words into file
    print("The number of unique words in description data set is %i." %len(words_stat.keys()))
    lowTF_words = set()
    with open(os.path.join(data_dir, 'words_statistics_description.txt'), 'w', encoding='utf-8') as f:
        f.write('TF\tDF\tWORD\n')
        for word, stat in sorted(words_stat.items(), key=lambda i: i[1], reverse=True):
            f.write('\t'.join([str(m) for m in stat[0:2]]) + '\t' + word +  '\n')
            if stat[0]<2:
                lowTF_words.add(word)
    print("The number of low frequency words of descriptions is %d." %len(lowTF_words))
    # print(stops)
    ...
    for description in descriptions:
        ...
        for w in words:
            if w not in lowTF_words:
                new.append(w)
        new_description = ' '.join(new)
        description_new.append(new_description)
        fout.write('%s\n' %new_description)
    fout.close()

    print("Description preprocessing is completed")
    ...
    ###Re-process samples, filter low frequency words...
    fout = open(os.path.join(data_dir, 'hashtag_processed.txt'), 'w', encoding='utf-8')
    hashtag_new = []
